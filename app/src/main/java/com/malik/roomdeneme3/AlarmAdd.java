...
public class AlarmAdd extends AppCompatActivity implements TimePickerDialog.OnTimeSetListener {
...
*/
    ...
    private void startAlarm(Calendar c) {
        ...
        if (intent2.hasExtra(EXTRA_NUM)) {
            pendnum=Integer.parseInt(intent2.getStringExtra(EXTRA_NUM));
    ...
    private void saveAlarm() {
        ...
        if (name.trim().isEmpty() || date.trim().isEmpty()) {
            Toast.makeText(this, "Please insert a name and date", Toast.LENGTH_LONG).show();
            return;
        }
        Calendar c = Calendar.getInstance();
